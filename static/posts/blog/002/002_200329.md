...
するのは手間がかかるので macOS の Automator を使ってツールを2つ作りました。
...
ディレクトリやファイル操作には Pathlib モジュールを使います。パスをオブジェクトとして操作できるようになります。 
...
定型文かつ、動的にデータを設定したい場合は String モジュールのテンプレートがおすすめ。
...
Automator で Python を実行するにはシェルから実行してもらうようにしないといけません。以下の順番でつなげていきます。
...
さらにどうせならと Automator で作ったアプリのアイコンも変えましょう。

![](./img/200329-3.jpg)

まず Automator で作成したアプリの情報ウィンドウを表示させます。次に左上のアイコンを選択状態にしてから、あらかじめクリップボードにコピーしておいた画像を command+v してあげれば OKです。
...
クイックアクションは macOS のサービス扱いになるため、システム環境設定から任意のショートカットキーを割り当ててあげましょう。
...
これで、アプリアイコンをダブルクリックするだけでブログが書けるようになりました。また、必要に応じてショートカットキーでプレビュー画面を起動する、ということもできました。
...
※勉強がてら flake8/pylint/black を PyCharm に導入してみました。pytest もやってようと思います。
